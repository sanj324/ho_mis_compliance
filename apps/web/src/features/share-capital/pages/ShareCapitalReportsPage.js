...
export const ShareCapitalReportsPage = () => {
    ...
    return (_jsxs("div", { className: "space-y-8", children: [_jsx(PageTitle, { title: "Share Capital Reports", subtitle: "Registers and Compliance Reports" }), _jsx(DataTablePlaceholder, { columns: ["Member Code", "Member Name", "Share Class", "No Of Shares", "Share Capital Value", "KYC Status"], rows: (shareRegister ?? []).map((item) => ({
                ...
                })) }), _jsx(DataTablePlaceholder, { columns: ["Member Code", "Member Name", "Share Class", "Dividend Amount", "Payment Status"], rows: (dividendRegister ?? []).map((item) => ({
                    "Member Code": String(item.memberCode ?? ""),
                    "Member Name": String(item.memberName ?? ""),
                    "Share Class": String(item.shareClass ?? ""),
                    "Dividend Amount": String(item.dividendAmount ?? ""),
                    "Payment Status": String(item.paymentStatus ?? "")
                })) }), _jsx(DataTablePlaceholder, { columns: ["Member Code", "Member Name", "Branch", "KYC Status"], rows: (kycDeficient ?? []).map((item) => {
